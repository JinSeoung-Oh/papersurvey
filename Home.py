...
- **상황별로 제일 첫 페이지에서는 주어진 상황에 대하여 스스로 생각하시는 중재 방안을 제시해주시면 됩니다. 해당 페이지는 왼쪽 페이지 리스트에서 wo 라는 태그가 앞에 붙어 있습니다. 해당 페이지에서는 중재 방안을 입력하시면 됩니다.**
...
- **상황별로 두 번째 페이지에서는 LLM 베이스의 system과 상호작용을 하시게 됩니다. 해당 페이지에는 왼쪽 페이지 리스트에서 w 라는 태그가 앞에 붙어 있습니다. 해당 페이지에서는 새롭게 제시된 상황을 요약해서 입력하시면 됩니다.**

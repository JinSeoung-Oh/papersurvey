...
def build_prompt_with_past_history(
    ...
    if cause_mode == "sensory":
        ...
        cause_rule = (
            ...
            "[의사소통 어려움 | 일상/전환 혼란 | 생리적 피로 | 감정 조절 실패 | 사회적 오해 | 학습된 반응]. "
...
- 시간 경과를 명시하고 (예: "10분 후", "한동안 지나자", "잠시 후"), 변화의 원인이나 과정을 포함하세요
...
**6. 새로운 요소 도입 원칙**
- **새로운 감각 자극 도입 시**: 현재 물리적 환경에서 합리적으로 발생 가능한 것만 선택
  (실내라면 실내에서 나올 수 있는 소리/냄새/촉감, 실외라면 실외 환경에 맞는 자극)
- **자극의 발생 과정을 명확히 서술**: "이때 ~가 시작되었다", "갑자기 ~이 들려왔다" 등
- **환경 논리성 확인**: 집 안인데 비가 직접 떨어지거나, 닫힌 공간에서 바람이 불거나, 
  밤인데 갑자기 강한 햇빛이 들어오는 등의 비논리적 상황 절대 금지

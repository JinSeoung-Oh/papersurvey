...
def build_prompt_with_past_history(
    ...
    if cause_mode == "sensory":
        cause_rule = (
            ...
            "(예: '이때 ~소리가 들리기 시작했다', '~냄새가 퍼져왔다', '~바람이 불어왔다' 등)"
        )
        transition_guide = (
        ...
        new_guide = ("**새로운 감각 자극 도입 시**: 현재 물리적 환경에서 합리적으로 발생 가능한 것만 선택 (실내라면 실내에서 나올 수 있는 소리/냄새/촉감, 실외라면 실외 환경에 맞는 자극)"
                     "**자극의 발생 과정을 명확히 서술**: '이때 ~가 시작되었다', '갑자기 ~이 들려왔다' 등"
                     "**환경 논리성 확인**: 집 안인데 비가 직접 떨어지거나, 닫힌 공간에서 바람이 불거나, 밤인데 갑자기 강한 햇빛이 들어오는 등의 비논리적 상황 절대 금지"
                     "시간 경과, 환경 변화, 내적 상태 변화 등으로 인한 자연스러운 전개 추구"
        )
    else:
        cause_rule = (
            "도전 행동의 원인은 **비감각(nonsensory) 요인** 중 정확히 1개를 선택하세요: "
            "[중재 방안 거부 | 생리적 피로 | 감정 조절 실패 | 사회적 오해 ]. "
            "**물리적 감각 자극을 원인으로 명시하지 마세요.** "
            "환경적 맥락이 필요하더라도 비감각적 요인으로 행동의 논리를 구성하세요."
        )
        transition_guide = (
            "중재 이후에도 **인지적 요인**으로 인한 어려움이 지속되거나 새롭게 발생할 수 있습니다. "
            "감각적 완화가 있었더라도 비감각적 원인으로 인한 도전 행동이 나타날 수 있습니다."
        )
        new_guide = ("**새로운 비감각적 상황 도입 시**: 현재 물리적 환경에서 합리적으로 발생 가능한 것만 선택 (실내라면 실내에서 가능한 상황, 실외라면 실외 환경에 맞는 상황)"
                     "**환경 논리성 확인**: 집 안인데 비가 직접 떨어지거나, 닫힌 공간에서 바람이 불거나, 밤인데 갑자기 강한 햇빛이 들어오는 등의 비논리적 상황 절대 금지"
                     "시간 경과, 환경 변화 등으로 인한 자연스러운 전개 추구"
        )
...
- 130~220자의 한 단락
- **자연스러운 전개 구조**: (중재 효과/시간 경과) → (전환 과정/변화 징조) → (새로운 상황 발생) → (개인 반응) → (구체적 행동)
...
- 눈에 띄는 불안이나 초조, 울음, 소리지르기, 자해, 공격성, 파괴적 행동, 반복 행동, 위험한 행동 등
- 단순히 불편함을 느끼고 합리적으로 해결하는 상황이 아님
- 자폐 특성으로 인한 부적응적 반응이 나타나야 함"
- 반드시 원인 설정 규칙과 상황 전개 가능성이 맞는 상황이어야만 합니다.
- 생성된 상황이 감각적인 원인에 의한 것인지 아니면 비감각적인 원인에 의한 것인지를 구분한 뒤, 원인 설정 규칙에 맞게 생성이 되었는지 검토하세요.
- 철저하게 관찰자의 입장에서 서술되어야 합니다. 상황 주체의 내면 상황 및 생각을 묘사하는 것은 바람직하지 않습니다.
- 상황의 주체는 자폐인입니다. 자폐인이라는 맥락에 맞는 상황이어야만 합니다. 

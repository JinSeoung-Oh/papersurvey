...
if "comments_history7" not in st.session_state:
    st.session_state.comments_history7 = []  # 전문가 중재 입력 이력

if "generated_situations7" not in st.session_state:
    st.session_state.generated_situations7 = []  # LLM 생성 상황 이력

if "loop_index7" not in st.session_state:
    st.session_state.loop_index7 = 0  # 0: 초기 중재 입력, 1~3: 생성 루프

# 사용자 프로필(예시)
USER_PROFILE_7 = {
    'sensory_profile': {'sound': 'high', 'light': 'high'},
    'comm_prefs': {'visual': 'low', 'verbal': 'medium'},
...
- **논리적으로 말이 안 되는 상황은 생성 금지**(예: 이불을 머리 끝까지 썼는데 하늘을 바라본다는 서술)
- 감각 원인을 고를 경우 **감각 1종만** 사용.
- '과거 히스토리'에서 유추할 수 있는 것이 아닌 뜬금없는 새로운 상황 생성은 금지

[생성 규칙]
1) 새 상황은 '직전 컨텍스트' 이후 자연스럽게 이어집니다(완화 실패/거부/부작용 가능).
2) 반드시 다음 지시를 따라주세요: {cause_rule}
3) 철저하게 관찰자 시점으로 서술, 전문가/중재/조언/평가 직접 언급 금지.
4) 130~220자, 한 단락.
5) 흐름: (중재 이후) → 인지/환경 변화 → 정서 변화 → 행동(관찰).
6) '과거 히스토리'와 '직전 컨텍스트'에서 얻을 수 있는 정보를 최대한으로 활용하여 '직전 컨텍스트'에서 바로 연속된 상황으로 생성
7) 생성된 상황은 논리적으로 자연스럽고 이해하기 쉬워야함

[출력]
- 위 조건을 만족하는 **자폐인의 도전 행동이 일어난 상황 서술 문단 1개**만 출력.
""".strip()
...
if st.session_state.loop_index7 == 0:
    ...
    if go:
        if comment.strip() == "":
            ...
            st.stop()
        st.session_state.comments_history7.append(comment)  # 초기 중재 저장
...
# -------------------------------
    ...
    if len(st.session_state.generated_situations7) < idx:
        # 직전 상황: 없으면 초기 설명 사용
        previous_situation = (
            st.session_state.generated_situations7[-1]
            if st.session_state.generated_situations7
            else "초기 멜트다운: 식당에서 배고픔과 피곤함에 자폐인이 멜트 다운을 일으키고 있음"
        )
        # “직전 상황에 대한” 전문가 중재(가장 최근 입력)
        expert_action_txt = st.session_state.comments_history7[-1]

        # ---- 과거 히스토리(전전~) 구성: 오래된 → 덜 오래된, 직전 페어 제외 ----
        history_pairs7 = []
        S = len(st.session_state.generated_situations7)
        # 상황 i에 대한 중재는 comments_history7[i+1]
        # 과거 범위: i = 0 .. S-2
        for i in range(max(0, S - 1)):
            s = st.session_state.generated_situations7[i]
            if i + 1 < len(st.session_state.comments_history7):
                a = st.session_state.comments_history7[i + 1]
                history_pairs7.append((s, a))
        ...
        # 프롬프트 빌드 + 호출
        prompt = build_prompt_with_past_history7(
            previous_situation=previous_situation,
            expert_action=expert_action_txt,
            user_profile=USER_PROFILE_7,
            history_pairs7=history_pairs7,
    ...
    with st.form(f"form_comment7_{idx}"):
        new_comment = st.text_area("이 상황에 적절한 중재 방안을 입력해주세요", key=f"comment7_{idx}")
        go_next = st.form_submit_button("다음")
    if go_next:
        if new_comment.strip() == "":
            st.warning("중재 방안을 입력해주세요.")
            st.stop()
        st.session_state.comments_history7.append(new_comment)  # 이번 상황에 대한 중재
        st.session_state.loop_index7 += 1
        st.rerun()
...
# -------------------------------
    ...
    if not st.session_state.survey_submitted7:
        ...
        # 상황 + '그 상황에 대한' 중재 저장
        # 규칙: situation_i ↔ comments_history7[i+1]
        with open(filepath, "a", encoding="utf-8") as f:
            for i, situation in enumerate(st.session_state.generated_situations7):
                if (i + 1) < len(st.session_state.comments_history7):
                    intervention = st.session_state.comments_history7[i + 1]
                    f.write(
                        f"{now},{expert_id},{i+1},"
                        f"\"{situation.strip()}\","

...
# 설문 제출 여부 상태 초기화
if 'survey_submitted' not in st.session_state6:
    st.session_state6.survey_submitted = False
...
if st.button("설문 제출"):
    ...
    st.session_state6.survey_submitted = True
    st.success("응답이 저장되었습니다. 감사합니다!")
    
if st.session_state6.survey_submitted:
    col1, col2 = st.columns([1, 1])
    with col1:
        if st.button("◀ 이전 페이지"):
            st.switch_page("pages/5_w_system_2.py")       # pages/home.py (확장자 제외)
    with col2:
        if st.button("다음 페이지 ▶"):
            st.switch_page("pages/7_wo_system_3.py")

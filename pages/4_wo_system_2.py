...
if 'llm' not in st.session_state4:
    st.session_state4.llm = _4oMiniClient()
...
if 'survey_submitted' not in st.session_state4:
...
if "comments_history" not in st.session_state4:
    st.session_state4.comments_history = []

if "generated_situations" not in st.session_state4:
    st.session_state4.generated_situations = []

if "loop_index" not in st.session_state4:
    st.session_state4.loop_index = 0

# 초기 질문만 출력
if st.session_state4.loop_index == 0:
    comment = st.text_area("주어진 상황에 대하여 가장 적절한 것으로 보이는 중재 방안을 입력해주세요", key="initial_comment")
    if st.button("다음"):
        if comment.strip() == "":
            st.warning("중재 방안을 입력해주세요.")
            st.stop()
        st.session_state4.comments_history.append(comment)
        st.session_state4.loop_index += 1
        st.rerun()

# 반복 상황 생성 루프
elif 1 <= st.session_state4.loop_index <= 3:
    idx = st.session_state4.loop_index

    # 상황 생성
    if len(st.session_state4.generated_situations) < idx:
        user_comment = st.session_state4.comments_history[-1]
        previous_situation = st.session_state4.generated_situations[-1] if st.session_state4.generated_situations else "초기 멜트다운: 등교길에 우연히 만난 토끼에 매우 놀란 모습을 보임. 울면서 불안한 모습을 보이고 있음"
        prompt = f"""다음은 자폐 아동의 멜트다운 상황입니다:
                  ...
                  """
        new_situation = st.session_state4.llm.call_as_llm(prompt)
        st.session_state4.generated_situations.append(new_situation)

    # 새로운 상황 제시 및 중재 방안 입력
    st.markdown(f"### 새로 생성된 상황 {idx}")
    st.markdown(st.session_state4.generated_situations[idx - 1])
    ...
    if st.button("다음", key=f"next_{idx}"):
        if new_comment.strip() == "":
            st.warning("중재 방안을 입력해주세요.")
            st.stop()
        st.session_state4.comments_history.append(new_comment)
        st.session_state4.loop_index += 1
        st.rerun()


elif st.session_state4.loop_index > 3:
    st.success("3회의 상황 생성 및 중재 응답이 완료되었습니다. 감사합니다.")

    if not st.session_state4.survey_submitted:
        # 자동 저장
        ...
        with open(filepath, "a", encoding="utf-8") as f:
            for i, (situation, intervention) in enumerate(zip(st.session_state4.generated_situations, st.session_state4.comments_history[1:]), start=1):
                f.write(
                    f"{now},{expert_id},{i},"
                    f"\"{situation.strip()}\","
        ...
        st.session_state4.survey_submitted = True
